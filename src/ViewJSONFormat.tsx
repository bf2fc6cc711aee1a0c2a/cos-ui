...
export const ViewJSONFormat: FunctionComponent = () => {
  ...
  const [showServiceAccount, setShowServiceAccount] = React.useState<boolean>(false);
  ...
  function maskPropertyValues(inputObj: any) {
    ...
    const json = JSON.stringify(inputObj, (key, value) => {
      return dataToHide.indexOf(key) === -1 ? value : "*".repeat(value.length);
    }, 2);
    return json;
  };

  const getJson = (properties: any, showHiddenFields: boolean) => {
    return showHiddenFields ? properties : maskPropertyValues(JSON.parse(properties));
  };
  ...
  const actions = (
    <React.Fragment>
      <CodeBlockAction>
        ...
        <Tooltip content={<div>{showServiceAccount ? 'Hide service account' : 'Show service account'}</div>} reference={showTooltipRef} />
      </CodeBlockAction>    
      <CodeBlockAction>
        <ClipboardCopyButton
          id="copy-button"
      ...
      <CodeBlockAction>
        <Button
          ...
          onClick={e => downloadFile(e, getJson(configString, showServiceAccount))}
        >
          <FileDownloadIcon />
        </Button>
  ...
  return (
    <CodeBlock actions={actions}>
      <CodeBlockCode id="code-content">{getJson(configString, showServiceAccount)}</CodeBlockCode>
    </CodeBlock>    
  )
}