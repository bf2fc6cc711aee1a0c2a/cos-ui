    ...
    function chooseStep(stepId: number) {
      switch (stepId) {
        case PROPERTIES_STEP_ID:
          return (
            <Properties configuration={props.configuration} onChange={(conf: unknown,status: boolean)=> props.onChange(conf,status)}></Properties>
          );
        case FILTER_CONFIGURATION_STEP_ID:
          return (
            <FilterDefinition configuration={props.configuration} onChange={(conf: unknown,status: boolean)=> props.onChange(conf,status)}></FilterDefinition>
          );
        case DATA_OPTIONS_STEP_ID:
          return (
            <DataOptions configuration={props.configuration} onChange={(conf: unknown,status: boolean)=> props.onChange(conf,status)}></DataOptions>
          );
        case RUNTIME_OPTIONS_STEP_ID:
          return (
            <RuntimeOptions configuration={props.configuration} onChange={(conf: unknown,status: boolean)=> props.onChange(conf,status)}></RuntimeOptions>
          );
        default:
          return <></>;
      }
